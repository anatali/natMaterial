    ...
    },
    {
        ...
        func: function (templatePath, inObj) {
            ...
            /* BYAN *** */ 
				...
				console.log("evaluate --- templatePath= " + templatePath + " templateLocation=" + templateLocation ); //+ " session=" + session
				//templateLocation=/usr/src/app/src/service-templates/hl7v2
               ...
               var partial = handlebarsInstance.partials[templatePath];
                ...
                if (typeof partial !== 'function') {
					...
					console.log( "READ FROM FILE" );  //BY AN
...
{ lookupProperty: [Function: lookupProperty],
  ...
  loc:
   { start: { line: 1, column: 8 }, end: { line: 1, column: 40 } } }
						*/			
						console.log(inObj.data.root.msg); 	//data & meta				
					}  
                }
...
//               return JSON.parse(jsonProcessor.Process(partial(inObj.hash)));
				...
				if( v.includes('male')  ) console.log(v);	//BYAN || v.includes('address')
